...
jQuery(function(c) {
    ...
    // Удален код бесконечной прокрутки для постов и комментариев
    ...
    c(".js-search-icon").on("click", function() {
    ...
    1 == settings_array.lightbox_display && c(".entry-content").find("a:has(img)").wpshoplightbox();
});
...
// Добавляем функцию показа мобильной рекламы moe.video
function showMobileMoeVideo() {
    if (document.getElementById('mvcontentroll')) return; // Если уже добавлена, не добавляем повторно
    
    const moeDiv = document.createElement('div');
    moeDiv.id = 'mvcontentroll';
    document.body.appendChild(moeDiv);
...
window.showMobileMoeVideo = showMobileMoeVideo;

document.addEventListener("DOMContentLoaded", (function() {
    addYandexRTBScripts();
    const e = window.innerWidth < 768,
        t = "/" === window.location.pathname || "/index.html" === window.location.pathname,
        n = document.querySelector(".post-box__category");

    if (e) {
        // Для мобильных устройств - показываем floorAd с колбэком
        window.yaContextCb.push(() => {
            Ya.Context.AdvManager.render({
                ...
                // Этот колбэк выполняется если реклама не отрисовалась
                window.showMobileMoeVideo();
            });
        });
    }
    ...
    window.yaContextCb.push((() => {
        Ya.Context.AdvManager.render({
            blockId: e ? "R-A-15627427-3" : "R-A-15627427-2",
            type: "fullscreen",
            platform: e ? "touch" : "desktop"
        })
    }));

    insertContentAds();
    
    // Показываем плавающую рекламу везде кроме главной и категорий на десктопе
    if (!e && !(t || n)) {
        initFloatingAds();
    }
}));
